...
/** Resolved theme ('dark' or 'light'), reactive to preference and system changes. */
export function useResolvedTheme(): 'dark' | 'light' {
  const { preference } = useThemeStore()
  const systemIsLight = useSyncExternalStore(
    subscribeToMediaQuery,
    getSystemIsLight,
    // Server snapshot: default to dark (not light) during SSR
    () => false,
  )
  return resolveTheme(preference, systemIsLight)
}

/**
 * Hook that applies the resolved theme to the document. Renders in the root
 * layout so it runs on every page.
 */
